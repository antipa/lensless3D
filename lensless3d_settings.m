...
nocrop = @(x)x;
nopad = @(x)x;
file_to_process = 'G:\Diffusers\Miniscope\Simulations\zebrafish_lenslets_sparsest.png';
impulse_stack = 'G:\Diffusers\Miniscope\Simulations\lenslets_miniscope_zemax_stack.mat';   %Mat file of impulse response
%impulse_stack = 'E:\data\uniform_lenslets_1387_zstack.mat';
fake_im = 'E:\data\fish_stack_isl1.mat';
stack_name = 'hstack';  %Variable name within impulse_stack
ds = 1/4;   %Lateral downsampling
dsz = 1/2;   %Axial downsampling
start_plane = 41;   %Which z plane to begin impulse response
end_plane = 100;
im_background = 0;
...
prox_handle = @(x)tvdenoise3d_wrapper(x,options.tau,niters,0,inf);
